...
@dataclass
class CharacterOverlayLandscapeConfig:
    image_path: str
    image_is_video: bool
    resize_ratio: float
...
class CharacterOverlayLandscape(VideoTemplate):
    Config = CharacterOverlayLandscapeConfig

    @classmethod
    def build_config(cls, config: Dict[str, Any], assets: Dict[str, Any]) -> CharacterOverlayLandscapeConfig:
        ...
        resize_ratio = coerce_number(
            ...
            pick_env_default(config, f"{prefix}_FORMAT_PICTURE_WIDTH_RATIO", 0.15),
        )
        position_x = coerce_number(
            pick_field(config, ("position_x",), None),
            pick_env_default(config, f"{prefix}_FORMAT_PICTURE_X_RATIO", 0.02),
        )
        position_y = coerce_number(
            pick_field(config, ("position_y",), None),
            pick_env_default(config, f"{prefix}_FORMAT_PICTURE_Y_RATIO", 0.78),
        )
        ...
        return CharacterOverlayLandscapeConfig(
    ...
    def render(self):
        ...
        image_x = width * self.config.position["x"]
        ...
        # MoviePy set_opacity does not accept a callable; build a mask instead.
        mask_clip = VideoClip(
